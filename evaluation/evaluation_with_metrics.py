...
def _calculate_metrics_with_details(gold_records, guess_records):
    """
    Calculate metrics and add detailed metrics to each prediction record.
    Returns both overall results and enhanced prediction records with metrics.
    """
    ...
    # Enhanced prediction records with metrics
    enhanced_guess_records = []
    ...
    for guess_item, gold_item in zip(guess_records, gold_records):
        ...
        if not gold_candidate_answers:
            # If no gold answers, still add the record but with zero metrics
            enhanced_record = guess_item.copy()
            enhanced_record["metrics"] = {
                "accuracy": 0.0,
                "em": 0.0,
                "cover_em": 0.0,
                "f1": 0.0,
            ...
            enhanced_guess_records.append(enhanced_record)
            continue
        ...
        # Calculate metrics for this specific prediction
        ...
        # Create enhanced record with metrics
        enhanced_record = guess_item.copy()
        enhanced_record["metrics"] = {
            "accuracy": local_accuracy,
            "em": local_em,
            "cover_em": local_cover_em,
            "f1": local_f1,
            "rougel": local_rougel
        }
        # Add ground truth to the enhanced record
        enhanced_record["ground_truth"] = list(gold_candidate_answers)
        enhanced_guess_records.append(enhanced_record)
    ...
    return overall_results, enhanced_guess_records
...
def evaluate_with_metrics(gold_filepath, guess_filepath, output_filepath=None):
    ...
    # 1. Calculate metrics and get enhanced records
    result, enhanced_guess_records = _calculate_metrics_with_details(gold_records, guess_records)
    ...
    # 3. Save enhanced records with metrics if output path provided
    if output_filepath:
        try:
            with open(output_filepath, "w", encoding="utf-8") as fout:
                for record in enhanced_guess_records:
                    fout.write(json.dumps(record, ensure_ascii=False) + "\n")
            print(f"Enhanced predictions with metrics saved to: {output_filepath}")
        except IOError as e:
            print(f"Error saving enhanced predictions: {e}")

    pp.pprint(result)
    return result, enhanced_guess_records


def evaluate_folder_with_metrics(guess_folder_path, gold_filepath, output_dir=None):
    """
    Evaluate all .jsonl files in specified folder and write enhanced predictions with metrics.
    """
    ...
    if output_dir:
        ...
        print(f"Enhanced predictions will be saved to: '{output_dir}'")

    all_results = []

    for filename in sorted(guess_files):
        guess_filepath = os.path.join(guess_folder_path, filename)
        output_filepath = os.path.join(output_dir, f"enhanced_{filename}") if output_dir else None
        
        print(f"\n--- Evaluating: {filename} ---")
        
        # Call evaluation function
        evaluation_results, enhanced_records = evaluate_with_metrics(gold_filepath, guess_filepath, output_filepath)
        ...
        summary = {
            "filename": filename,
            "em": downstream_metrics.get("em", "N/A"),
            "cover_em": downstream_metrics.get("cover_em", "N/A"),
            "f1": downstream_metrics.get("f1", "N/A"),
            "accuracy": downstream_metrics.get("accuracy", "N/A"),
            "rougel": downstream_metrics.get("rougel", "N/A"),
            "evaluated_count": downstream_metrics.get("evaluated_count", "N/A"),
            "llm_judged_accuracy": llm_metrics.get("accuracy", "N/A") if llm_metrics else "N/A",
            "enhanced_records_count": len(enhanced_records) if enhanced_records else 0
        }
    ...
    if all_results:
        ...
        for result in all_results:
            ...
            print(f"  Cover EM: {result['cover_em']:.4f}")
            print(f"  F1: {result['f1']:.4f}")
            print(f"  ROUGE-L: {result['rougel']:.4f}")
            print(f"  Evaluated Count: {result['evaluated_count']}")
            if result['llm_judged_accuracy'] != "N/A":
                print(f"  LLM Judged Accuracy: {result['llm_judged_accuracy']:.4f}")
            print(f"  Enhanced Records: {result['enhanced_records_count']}")

    return all_results


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate QA predictions with detailed metrics per prediction.")
    ...
    parser.add_argument("--output_file", help="Output file path for enhanced predictions with metrics (JSONL format)")
    parser.add_argument("--guess_folder", help="Folder path containing multiple prediction files")
    parser.add_argument("--output_dir", help="Directory to save enhanced predictions with metrics (when using --guess_folder)")

    args = parser.parse_args()
    
    if args.guess_file:
        print(f"Evaluating single file '{args.guess_file}' against '{args.gold_file}'...")
        evaluation_results, enhanced_records = evaluate_with_metrics(args.gold_file, args.guess_file, args.output_file)
        ...
        if evaluation_results:
            ...
            if enhanced_records and args.output_file:
                print(f"Enhanced predictions with metrics saved to '{args.output_file}'")
                print(f"Total enhanced records: {len(enhanced_records)}")
        else:
            print("Evaluation failed or produced no results.")

    elif args.guess_folder:
        evaluate_folder_with_metrics(args.guess_folder, args.gold_file, args.output_dir)
